...
| Whether a Gaston cancel took | `data.confirmed` on `event = skill.gaston.pass` — the tsum count dropped by `cancelDrop` after the tap (null: a chain too short to check); `data.cleared` is how far it fell and `data.softTaps` the remembered bubbles tapped when the others did not take. `data.missed` on `skill.gaston.done` counts the unconfirmed ones, `data.extra` the other colours' chains a short pass drew, and `data.spamBudgetMs` how long the charge was allowed |
