...
| Whether a Gaston drag caught its chain stalling | `data.rewinds` on `event = skill.gaston.pass` — each walk back as `[route index the finger was on, index it went back to]` (`rewind`: no gold coin on the four tsums behind the finger); `data.coins` is the last check's read over the route from `coinFrom`, `C` a tsum carrying the game's coin, to set against the linked count on a recording |
