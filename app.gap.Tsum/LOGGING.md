...
| Whether a Gaston cancel took | `data.confirmed` on `event = skill.gaston.pass` — the tsum count dropped by `cancelDrop` after the tap (null: a chain too short to check); `data.cleared` is how far it fell and `data.softTaps` the remembered bubbles tapped when the others did not take, `data.freshTaps` those a read after the release found. `data.spared` marks a chain under `cancelMinChain`, released uncancelled so the bubbles go to the next long one (never with `surplusBubbles` on the board). `data.popped` on `skill.gaston.done` counts bubbles popped on passes that drew nothing. `data.missed` on `skill.gaston.done` counts the unconfirmed ones, `data.extra` the other colours' chains a short pass drew, and `data.spamBudgetMs` how long the charge was allowed |
| Whether a Gaston drag caught its chain stalling | `data.rewinds` on `event = skill.gaston.pass` — each walk back as `[route index the finger was on, index it went back to]` (`rewind`: no gold coin on the four tsums behind the finger), with a third entry when the same stall came back: the tsums a fresh route from there added, or 0 where the drag stopped (no route, or no time before the close); `data.coins` is the last check's read over the route from `coinFrom`, `C` a tsum carrying the game's coin, to set against the linked count on a recording |
| When Gaston's held chain let go | `data.releaseLeadMs` on `event = skill.gaston.done` — release against the antlers leaving the chrome (`antlerReleaseMs` is the hold working; under 0 is a release inside the window, which charges nothing); `data.antlersOnMs`/`data.antlersOffMs` are when the antlers came up and went, from the tap (0: not seen), and `data.closeAtMs` the close the window planned on (`antlerMs`). `data.trimmed` on `event = skill.gaston.pass` is how many tsums were cut off the closing chain so it ends `closeLeadMs` before the close |
