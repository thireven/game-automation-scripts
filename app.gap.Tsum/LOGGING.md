...
| When Gaston's board opened | `data.openMs` on `event = skill.gaston.done` — the opening gate, from the tap; `data.openBlind` true when the board read full on every poll from the tap (a charge fired inside the last closing chain's pop), so the gate held to `openBlindMs` rather than `openMinMs` |
