...
| Wait for Settle (ms)               | The most time the script leaves the board alone once the skill gauge reads full, before it taps the skill (0-3000 in steps of 200, default 0 — fire at once). The script chains fast enough that the gauge usually fills while much of the board is still empty and refilling, and a skill fired then clears very little. It watches the board and fires as soon as the tsums have stopped falling, so a generous value costs nothing on a board that has already refilled; only a board still moving at the deadline spends the whole of it. With it set, burst skills stop being tapped blind between chains and go through the same gauge check as every other skill, so the wait can land before the tap. Changes take effect on the next activation. |
...
| Auto Unlock MyTsum Level           | When the level-up screen after a round shows **Raise level cap!** on your MyTsum (the first card), the script goes to the Tsum list straight after that round and buys that one raise (attention: consumes coins), then plays on as normal. Only the selected tsum, only when the game says it is capped, and nothing is bought unless the list opens on the MyTsum with the raise on offer. If the raise does not go through — usually not enough coins — it waits half an hour before trying again. Independent of the sweep above.                                                                                                                                                                                                                                                                      |
