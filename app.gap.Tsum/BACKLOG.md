...
- Coronation Day Elsa's window is 10s at level 6 from the end of the activation animation (2026-09-13), but where exactly it closes is not measured: chains 12.0s after the tap froze in `coronation_elsa_5.mp4`, and the final slot sits at 11.0s on the assumption that the animation is ~1.5s. A chain that misses the close costs the whole-board freeze, so a recording of the final chain landing is the first thing to check. Also open: whether the freeze charge counts from the activation or from the previous freeze (a hand-played window with chains at 1s, 5s and 9s settles it), and whether the bomb the break spawns is where the post-burst look's bubble pop lands. The first log to read after a run is `skill.elsa.pass` -- `chainLen` 3-5 with the video showing the chain's count reaching that number is the chains registering, which they never did before `elsaStripChains`.
