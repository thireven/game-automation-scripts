...
- Coronation Day Elsa's row sweep is unverified on a device (2026-09-12): it was written off two recordings and a player's write-up of the skill, never run. The first log to read is `skill.elsa.done` -- `chains` should be six to eight a window and `closedEarlyMs` zero at level 6; a non-zero `closedEarlyMs` on every window means the real window is shorter than `durationMs` says -- and `skill.elsa.pass` says which row each chain stood on and how much ice the look before it read. Also unmeasured: whether the band is thick enough that `rowTolerance` (14) skips rows the band already took, and whether the bomb the break spawns is where the post-burst look's bubble pop lands.
