...
- Coronation Day Elsa's freeze charge is not pinned down (2026-09-12): the band thickens with time in the window and is the whole board at ~10s, but whether the charge runs from the activation or from the previous chain is open, and `chainAt` is set for either. One recording settles it -- chains at 1s, 5s and 9s of one window: a whole-board band at 9s says activation (add slots before the last), a half-board one says previous chain (keep two, or drop to one). Also unverified on a device: the scheduled window itself (`skill.elsa.done` -- `chains` 2, `missed` 0, `icedLast` most of the board), and whether the bomb the break spawns is where the post-burst look's bubble pop lands.
